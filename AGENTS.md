...
- **Session 2026-09-24 — FCM push fixed (secrets), per-stream chat isolation, single-stream-per-church, communities events, releases +345/+346, Africa map probe, city z16-19 pipeline**:
  ...
  - **PENDING**: (1) **rotate the FCM service-account key** (shared in plain text); (2) actually build city tiles on a bandwidth-good machine (`build-city-tiles.ps1 -Cities lusaka,harare`) then uncomment `MAPS_EXTRA_SOURCES` in `.env`; (3) add ticketing deep-link routes `/ticket/:id`, `/events/:id/tickets`, `/events/:id/manage-tickets` (still missing).
