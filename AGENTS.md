...
- **Session 2026-09-24 — FCM push fixed (secrets), per-stream chat isolation, single-stream-per-church, communities events, releases +345/+346, Africa map probe, city z16-19 pipeline**:
  ...
  - **Releases**: APK **v1.0.0+345** (221.6 MB) + AAB **v1.0.0+346** (127 MB) on R2 (`builds/latest/` + versioned + `latest.json`); `+343`/`+344` pruned. Web deployed (`44cdd08b.churchonapp.pages.dev`). main pushed.
