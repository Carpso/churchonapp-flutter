...
- **Session 2026-09-24 — FCM push fixed (secrets), per-stream chat isolation, single-stream-per-church, communities events, releases +345/+346, Africa map probe, city z16-19 pipeline**:
  - **PUSH NOTIFICATIONS FIXED (was the only remaining blocker)**: `FCM_PROJECT_ID` + `FCM_SERVICE_ACCOUNT` were **never set** in Supabase (verified via health probe before the fix). Set from the Firebase service-account JSON the user provided (`studio-7483333628-db257`, SA `firebase-adminsdk-fbsvc@…`, key id `9d33eda9b4cc8a61084bfe829675ec0df6757525`) via `supabase secrets set --env-file` (compacted to a one-line dotenv, **never written to the repo**, temp file deleted), `push-notifications` redeployed. Permanent secret-free probe: `GET .../functions/v1/push-notifications?health=fcm` -> `{"fcm_project_id_set":true,"fcm_service_account_set":true,"service_account_parses":true,"project_ids_match":true}`. **⚠ The key was shared in plain text — ROTATE it in Firebase.**
  - **Livestream chat NOT per-stream (ROOT CAUSE)**: viewer chat was keyed on `live_chat_messages.tenant_id` (one chat per CHURCH) and the realtime channel filtered `.eq('tenant_id', …)`, so **every broadcast shared one chat and ended-stream messages reappeared in the next stream**; the table had no `stream_id` at all. Migration **`20261233_stream_chat_isolation.sql`** (applied): adds `stream_id` FK, backfills legacy rows to the same-church stream whose window contains `created_at`, index `(stream_id, created_at)`, replaces the blanket `USING(true)`/`WITH CHECK(true)` policies with stream/church-scoped SELECT+INSERT, adds the table to `supabase_realtime` + `REPLICA IDENTITY FULL`. Client: `liveChatStreamProvider` keyed by `stream_id`, subscription `.eq('stream_id', id)`, messages written with `stream_id`+`tenant_id`+`user_id`, chat clears on stream change, input disabled with "This stream has ended" for `ended`/`archived`/replay.
  - **One active stream per church**: `20261231_one_active_stream_per_church.sql` — heals duplicates, partial unique index `(church_id) WHERE status='live'`, RPCs `get_active_stream_for_church`, `stop_other_streams`, `start_stream_guard`. UI: "Another stream is already live — stop it and start yours?" STOP & START / CANCEL in studio + admin.
  - **WHEP viewer playback**: Cloudflare does NOT emit HLS/DASH for a WHIP/WebRTC broadcast (the fabricated `…/<input_uid>/manifest/video.m3u8` returns **HTTP 204**). Viewers now play **WHEP** (`WhepPlayback`, flutter_webrtc recvonly) with a bounded "waiting for broadcast" poll. `cloudflare-stream` `normaliseLiveInput` returns `{input_status, connected, enabled, hls, dash, preview, whep, mode}`; `refresh_live_input`/`get_live_input` persist authoritative URLs.
  - **Communities events**: migration `20261232_community_events.sql` (Connect tab exception fixed).
  - **Releases**: APK **v1.0.0+345** (221.6 MB) + AAB **v1.0.0+346** (127 MB) on R2 (`builds/latest/` + versioned + `latest.json`); `+343`/`+344` pruned. Web deployed (`44cdd08b.churchonapp.pages.dev`). main pushed. **Fresh clean rebuild after chat/map/ticketing work: APK `v1.0.0+347` (223.0 MB) + AAB `v1.0.0+348` (127.6 MB)** on R2, `+345`/`+346` pruned, `latest.json` updated, pubspec pushed (`51b62b8`).
  - **Africa basemap probe — FAILED 5x, honest findings**: downloading the full Africa z15 extract (~15 GB) from `build.protomaps.com` from this box never completed (HTTP/2 `PROTOCOL_ERROR` ×2, TCP `wsarecv` timeout, user abort, and throttling to 772 B/s); the referenced daily build `20260917` also **404'd — Protomaps rotates/deletes old daily builds** (only recent ones exist; current `20260924` planet = 128.8 GB). **Do NOT pull multi-GB files onto local Windows — run `pmtiles extract` on a cheap cloud VM next to the data and push straight to R2.** Garbage partial file deleted; D: back to 174 GB free.
  - **HARD CAPABILITY FACTS (never re-litigate these)**: Protomaps public build = **maxzoom 15, NO buildings**. "Building level" (z16–19, what Yango/Waze look like) requires self-hosting **planetiler** on OSM PBF; continental z16–19 is **TB-scale** → do it **per city**. **No free global live-traffic feed exists** (Waze for Cities / TomTom / HERE are paid) — our own crowd-sourced driver data is the free alternative.
  - **Rental hosting architecture (documented in `docs/MAPS.md` §6)**: **R2 = tile data** (static PMTiles/sprites/fonts/styles; `$0.015/GB/mo`, egress free); **Cloudflare Workers = the metered API gateway with per-tenant keys, usage metering and billing — that is what you rent**; **routing (OSRM/Valhalla) + geocoding (Photon) need Cloudflare Containers or a small VM** — R2 cannot run compute. Growth: dated object snapshots + scheduled weekly refresh (`refresh-maps.ps1`).
  - **City z16–19 pipeline shipped (`scripts/map/`, commit `17996a9`)**: `metros.json` (4 countries + 6 metro bboxes: Lusaka, Ndola, Kitwe, Livingstone, Harare, Bulawayo), `r2-put.mjs` (S3 SigV4 PUT/HEAD/COPY, env creds, `node --check` clean), `build-city-tiles.ps1/.sh` (Geofabrik PBF download -> planetiler `--minzoom=13 --maxzoom=19` per metro -> R2 `tiles/<name>.pmtiles` stable + dated keys -> `build-report.json` -> paste-ready `MAPS_EXTRA_SOURCES=` line), `refresh-maps.ps1/.sh` (manifest `tiles/latest.json` + printed `schtasks`/cron lines). **The `.gitignore` was blocking `/scripts/*` — now un-gitignored for `scripts/map/`.**
  - **App multi-source switching**: new `lib/core/widgets/maps/map_sources.dart` (declarative region table + `resolveMapSource`, **smallest-bbox-wins**, base fallback) wired into `church_map.dart` (re-resolve on camera move, `ValueKey(_activeUrl)`, per-source `maximumZoom` 15→19, offline-cache sweep to active top zoom, silent city→base fallback, base-only RETRY chip). Config: `Env.mapsExtraSources` reading **`MAPS_EXTRA_SOURCES`** (JSON in `.env`) — **new cities/apps (Carpso Ride) need no code change**. `.env.example` corrected (old `zambia.pmtiles`/`zimbabwe.pmtiles` were deleted from R2). `map_sources_test.dart` 12/12.
  - **Docs**: `docs/MAPS.md` (capability matrix, build/refresh/switching, rental stack, constraints) updated; AGENTS.md + CHANGELOG + README updated this session.
  - **PENDING**: (1) **rotate the FCM service-account key** (shared in plain text); (2) actually build city tiles on a bandwidth-good machine (`build-city-tiles.ps1 -Cities lusaka,harare`) then uncomment `MAPS_EXTRA_SOURCES` in `.env`; (3) ~~ticketing deep-link routes~~ DONE 2026-09-24: `/ticket/:id`, `/events/:id/tickets`, `/events/:id/manage-tickets` already registered + guarded (`event_by_id_screens.dart` thin fetchers, `hasAccess` leadership gate on manage-tickets); share links (QR payload + e-ticket share) and universal-link handler verified to resolve; added missing `ticket`/`/event_ticket` push payload cases -> `/ticket/:id` (commit `321aa0b`).
  - **`flutter analyze lib`**: 0 errors, 0 warnings (3 pre-existing test infos) throughout.