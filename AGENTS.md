...
- **Session 2026-08-15 — Smart CI/CD Pipeline live**:
  ...
  - **⚠ USER ACTIONS PENDING**: (1) GitHub billing — "recent account payments have failed or spending limit needs to be increased" blocks ALL new runner allocations (Play Store deploy, App Distribution, notify jobs died 0-steps/2-3s) — fix in Settings → Billing & plans, then re-run; (2) add `FIREBASE_SERVICE_ACCOUNT` (JSON with App Distribution Admin) + `FIREBASE_TESTER_GROUPS` for OTA; (3) optional `SLACK_WEBHOOK_URL`/`DISCORD_WEBHOOK_URL`; (4) `GCLOUD_SERVICE_ACCOUNT` + `FIREBASE_PROJECT_ID` (`studio-7483333628-db257`) for Test Lab; (5) Google Cloud Console: add `https://churchonapp.com` as authorized JS origin (web OAuth); (6) verify web white screen in a real browser.
