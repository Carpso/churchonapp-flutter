...
| 2026-09-06 | Reader RPCs (`get_church_member_counts`, `get_church_monthly_tithes`, `get_prophetic_heatmap_data`, `get_prophetic_heatmap_legacy`) callable by anon | `auth.uid()`-gated (leader-of-tenant or COA staff) + `SET search_path`; anon gets empty result (migration `20261014`) |
| 2026-09-06 | Helper/trigger functions executable by anon/PUBLIC (`is_super_admin`, `is_admin_or_employee`, `is_church_pastor`, `approve_role_assignment`, `elevate_user_role`, `create_organization`, `sync_role_to_auth_metadata`, `sync_community_group_member_count`) | `REVOKE EXECUTE FROM PUBLIC/anon`, `GRANT authenticated/service_role`; triggers continue to fire (migration `20261014`) |
| 2026-09-06 | 18 tables had `{public}`-role RLS policies (anon readable/writable) | Scoped to `{authenticated}`: emergency_contacts (tenant-aware), notifications, audit_logs, service_reports, donor_segments (church-scoped), driver_locations, growth_heatmap_data, fundraising_contributions, group_contribution_members/payments, group_members, events, sermons, social_posts, klips, church_storage_usage (UPDATE→service_role) (migration `20261014`) |
| 2026-09-06 | Permissive public-role policies (Anyone can view social posts/contributions/group payments; Users can join groups / contribute to groups; anyone can insert klips/events/notifications) | Dropped or role→authenticated (migration `20261014`) |
| 2026-09-06 | `churches` had public-role "Anyone can register a church" INSERT with `WITH CHECK true` | Policy dropped; registration stays authenticated-only (migration `20261013`) |
| 2026-09-06 | Intentional public/anon access verified intact after remediation | 3 anon policies remain (church discovery `anon_discover_verified_churches`, expansion leads, sample sermons); bible/radio/quiz leaderboards/prayers/klips reads untouched |
