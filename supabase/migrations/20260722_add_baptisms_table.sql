CREATE TABLE IF NOT EXISTS baptisms (
  ...
  tenant_id TEXT REFERENCES churches(id),
...
CREATE POLICY "Users can view baptisms in their tenant"
  ON baptisms FOR SELECT
  USING (
    tenant_id = (SELECT tenant_id FROM profiles WHERE id = auth.uid())
    OR auth.uid() IN (SELECT id FROM profiles WHERE role = 'superadmin')
  );

CREATE POLICY "Users can insert baptisms"
  ON baptisms FOR INSERT
  WITH CHECK (
    auth.uid() = created_by
    AND (
      tenant_id = (SELECT tenant_id FROM profiles WHERE id = auth.uid())
...
CREATE POLICY "Admins can update baptisms in their tenant"
  ON baptisms FOR UPDATE
  USING (
    EXISTS (
