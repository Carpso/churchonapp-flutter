      ...
      try {
        ...
        _hls!.on(
            ...
            (String _, JSObject data) {
              // `data` is hls.js's plain JS error payload. It MUST be read
              // through `ErrorData`'s guarded `getProperty` access — Dart
              // dynamic dispatch on a JS object throws
              // `NoSuchMethodError: method not found: 'type'`, and that throw
              // used to happen inside this handler, swallowing the real
              // playback failure (the player never reached `initialized`).
              ErrorData errorData;
              try {
                errorData = ErrorData(data);
              } catch (e) {
                // Belt-and-braces: the handler itself must never throw.
                debugPrint('Error parsing hlsError: $e');
                errorData = ErrorData(null);
              }
              if (errorData.fatal) {
                // Surface as a normal player error so `initialize()`/listeners
                // fail fast instead of waiting on a manifest that will not load.
                _eventController.addError(PlatformException(
                  code: _kErrorValueToErrorName[2]!,
                  message: errorData.type.isNotEmpty
                      ? errorData.type
                      : 'HLS_FATAL_ERROR',
                  details: errorData.details.isNotEmpty
                      ? errorData.details
                      : _kErrorValueToErrorDescription[4],
                ));
              } else {
                debugPrint('hls.js non-fatal error: '
                    '${errorData.type}/${errorData.details}');
              }
            }.toJS);
