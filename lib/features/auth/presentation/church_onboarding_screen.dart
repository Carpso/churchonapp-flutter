...
class _ChurchOnboardingScreenState
  ...
  final _logoUrlController = TextEditingController(
    text: "https://churchonapp.com/churchonappicon.png",
  );
  ...
  Future<void> _submit() async {
    ...
    if (name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Church Name cannot be empty"),
    ...
    try {
      ...
      // Step 1: Create the parent tenants record
      final tenantRes = await supabase
          .from('tenants')
          .insert({'name': name, 'type': 'church', 'country': _currentCountry})
          .select('id')
          .single();
      final tenantId = tenantRes['id'] as String;

      // Step 2: Create the church record with tenant_id
      ...
      // Step 3: Update the registering user's profile with tenant_id
      ...
      String? inviteCode;
      try {
        final codeGen = CodeGeneratorService(supabase);
        inviteCode = await codeGen.generateTenantCode(_currentCountry);
        await codeGen.registerCode(
          codeType: 'tenant',
          codeValue: inviteCode,
          countryIso: CodeGeneratorService.countryToISO(_currentCountry),
          userId: user?.id,
          metadata: {'tenant_id': tenantId, 'church_name': name},
        );
      } catch (e) {
      ...
      if (mounted) {
        _showSuccess(data, inviteCode: inviteCode);
      }
    } catch (e) {
      if (mounted) {
  ...
  void _showSuccess(Map<String, dynamic> church, {String? inviteCode}) {
    showDialog(
      context: context,
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(
                "Welcome to the Ecosystem! ${_nameController.text} has been onboarded with a 30-day FREE trial.",
                        ...
                        child: ElevatedButton.icon(
                          onPressed: () {
                            ...
                            SharePlus.instance.share(
                              ShareParams(
                                text: "Join our church $_nameController.text on Church On App!\n\nUse invite code: $inviteCode\nOr open: $link",
                              ),
                            );
                          },
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildTextField(
              _nameController,
              "e.g. Grace Chapel International",
              Icons.church,
              (v) {
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildTextField(
              _treasurerPhoneController,
              "e.g. +260 966 ... (MTN/Airtel)",
          ...
          Expanded(
            ...
            child: ElevatedButton(
              ...
              child: _isSubmitting
                  ...
                  : Text(
                      _currentStep == 2 ? "FINALIZE ONBOARDING" : "NEXT STEP",
