...
class HomeEventTimeline extends ConsumerWidget {
      ...
      children: [
        eventsAsync.maybeWhen(
          data: (events) {
            ...
            return HomeSectionTitle(
              title: upcoming.isEmpty ? "Recent Events" : "Upcoming Events",
              trailing: "VIEW ALL",
              onTrailingTap: () => context.push('/events'),
            );
          },
          orElse: () => HomeSectionTitle(
        ...
        eventsAsync.when(
          ...
          data: (events) {
            ...
            // Fall back to the most recent events so the section is never
            // empty when a church has no scheduled future events yet.
            final past = events.where((e) => !e.date.isAfter(now)).toList()
              ..sort((a, b) => b.date.compareTo(a.date));
            final isPast = upcoming.isEmpty;
            final display = (isPast ? past : upcoming).take(3).toList();
            if (display.isEmpty) {
              return Container(
                ...
                child: Row(
                  children: [
                    ...
                    Text("No events yet", style: TextStyle(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.5), fontSize: 13)),
                  ],
                ),
              );
            }
            return Column(
              children: display
                  .map((e) => _buildEventItem(
                        context,
                        e.title,
                        isPast ? _formatPastTime(e.date) : _formatEventTime(e.date),
                        _getEventIcon(e.category),
                        imageUrl: e.imageUrl,
                      ))
                  .toList(),
            );
  ...
  String _formatPastTime(DateTime date) {
    final now = DateTime.now();
  ...
  Widget _buildEventItem(BuildContext context, String title, String time, IconData icon, {String? imageUrl}) {
    return Semantics(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            // Event banner image when available, else the category icon tile.
              ...
              ClipRRect(
                ...
                child: AppImage(
                  ...
                  errorWidget: (c, _) => Container(
                    width: 56,
                    height: 56,
                    color: Theme.of(c).primaryColor.withValues(alpha: 0.1),
                    child: Icon(icon, color: Theme.of(c).primaryColor, size: 20),
                  ),
                ),
              )
            else
