        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (_includeAttendance) _buildMetricRow(LucideIcons.users, "Attendance", "${data.attendance}", Theme.of(context).primaryColor),
              if (_includeGiving) _buildMetricRow(LucideIcons.banknote, "Giving", "K ${data.giving.toStringAsFixed(2)}", Colors.green),
              if (_includeNewMembers) _buildMetricRow(LucideIcons.userPlus, "New Members", "${data.newMembers}", Theme.of(context).primaryColor.withValues(alpha: 0.7)),
              if (_includeEvents) _buildMetricRow(LucideIcons.calendarCheck, "Events", "${data.events}", Colors.orange),
              if (_includeSermons) _buildMetricRow(LucideIcons.mic2, "Sermons", "${data.sermons}", Theme.of(context).primaryColor.withValues(alpha: 0.55)),
            ],
          ),
        ),
        const SizedBox(height: 15),
