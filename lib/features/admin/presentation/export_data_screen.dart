...
class _ExportDataScreenState extends ConsumerState<ExportDataScreen> {
  ...
  Future<void> _exportData(_ExportType type, Tenant tenant) async {
    ...
    try {
      ...
      final pdfBytes = await _exportService.buildPdfBytes(
        title: title,
      ...
      // WEB: no writable filesystem — share the generated bytes directly.
      if (dirPath == null) {
        await SharePlus.instance.share(ShareParams(
          files: [
            XFile.fromData(pdfBytes,
                mimeType: 'application/pdf', name: '$title.pdf'),
            XFile.fromData(
              Uint8List.fromList(utf8.encode(await _exportService.exportToCsv(rows, columns))),
              mimeType: 'text/csv',
              name: '$title.csv',
            ),
            XFile.fromData(
              Uint8List.fromList(utf8.encode(await _exportService.exportToJson(rows))),
              mimeType: 'application/json',
              name: '$title.json',
            ),
          ],
          text: '${tenant.name} data export',
        ));
        if (mounted) setState(() => _generating = false);
        return;
      }

      final pdfPath = '$dirPath/$title.pdf';
