...
**v1.0.0+348 — 1.0.0 | Flutter 3.35.1 | 0 errors, 0 warnings | 484/484 tests green | September 2026**
...
- **Media & Streaming** — Two-layer sermon media: **Cloudflare Stream** for playback (live inputs via RTMP/OBS or WebRTC WHIP; Direct Creator Upload transcodes uploaded sermons into adaptive-bitrate HLS with auto thumbnails) + **R2 master archive** (`sermons.archive_url`) so the source media is always owned. **YouTube playback** for tenants that publish sermons/streams on their YouTube channel (embedded player + fullscreen). **Audio sermons** (`just_audio` stage, mp3/m4a/wav upload). **HLS on web** via self-hosted `hls.min.js` (Chrome/Firefox, not just Safari). **Viewership analytics** (`sermon_views` + `record_sermon_view` RPC, 1/user/6h dedup; `viewer_count` on live streams). Live `live_streams.status` accepts `archived`. **Cloudflare Stream is the single streaming backend** (legacy MediaMTX removed). Kingdom Radio
- **Bible Study** — Reading plans, verse of the day, deep study suite, memory verses; NKJV/NLT translations, full-text search, verse notes/bookmarks, AI-powered chapter summaries via Kael. **Genesis 33 + Cross-References** — curated inter-testament cross-reference seed (harmony parallel accounts, prophecy→fulfillment, thematic links) with unique pair indexing, bidirectional lookup (either side of a pair finds its counterpart), and a Kael-AI fallback generator that persists new links via the authenticated insert policy. **Parallel Bible Reader** — chapter-level side-by-side comparison across 11 resolvable translations (KJV/WEB/ASV/BBE/YLT/DRA/Noyes/Tyndale/Webster/UKJV/MKJV), per-verse translation blocks, tap-to-add translations, route `/bible/:book/:chapter/parallel`.
